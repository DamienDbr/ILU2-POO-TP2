		...
		if(!controlIdAcheteur)
			...
			for(int i=2;i<longTabVendeurs;) {
				...
				if(Objects.equals(tabVendeurs[i], nomProduit)) {
					...
					tabVendeursProduit[indiceTabVendeursProduit] = tabVendeurs[i-2]; //On ajoute le nom du Vendeur au tab de vendeurs qui vendent le produit
			...
			if(testProduitVenduOuNon==0)
				...
				do {
					...
					if((numMarchand<0)||((numMarchand-1)>indiceTabVendeursProduit)){
						...
						System.out.println("Je suis désolée "+nomMarchandChoisi+" mais il faut être un habitant de notre village pour commercer ici. \n");
					
				}while((numMarchand<0)||((numMarchand-1)>indiceTabVendeursProduit)||!controlAcheterProduit.verifIdentVendeur(nomMarchandChoisi));
				
				
				System.out.println(nomAcheteur+" se déplace jusqu'à l'étal du vendeur "+nomMarchandChoisi+"\n");
				int nbAAcheter = Clavier.entrerEntier("Bonjour "+nomAcheteur+".\n"+"Combien de "+nomProduit+" voulez vous acheter ? \n");
				...
				if(quantiteRestante==0)
					System.out.println(nomAcheteur+" veut acheter "+nbAAcheter+" "+nomProduit+", malheureusement il n'y en a plus. \n");
				else if(quantiteRestante<nbAAcheter) {
					System.out.println(nomAcheteur+" veut acheter "+nbAAcheter+" "+nomProduit+", malheureusement "+nomMarchandChoisi+" n'en a plus que "+quantiteRestante+"."+nomAcheteur+" achète tous le stock de "+nomProduit+". \n");
					//Diminuer le stock de quantiteRestante
					controlAcheterProduit.diminuerQuantiteProduit(quantiteRestante, nomMarchandChoisi);
				}
				else {
					System.out.println(nomAcheteur+" achète "+nbAAcheter+ " "+nomProduit+ " à "+nomMarchandChoisi+".\n");
